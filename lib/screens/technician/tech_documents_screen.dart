...
import 'tech_documents_file_stub.dart'
    if (dart.library.html) 'tech_documents_file_web.dart' as file_helper;
...
class _TechDocumentsScreenState extends State<TechDocumentsScreen> {
  ...
  void _pickFile({
    ...
    if (!kIsWeb) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('File upload is available on the web version.')),
      );
      return;
    }
    file_helper.pickFile(
      accept: accept,
      capture: capture,
      onPicked: onPicked,
      onError: (msg) {
  ...
  void _downloadFile(String? base64, String? name) {
    ...
    if (!kIsWeb) return;
    file_helper.downloadFile(base64, name);
  }
    ...
    try {
      ...
      if (user != null) {
        ...
        final workerData = {
          ...
          'serviceType': DocumentDatabase.currentCategory,
          'experienceLevel': DocumentDatabase.currentExperience,
          'hourlyRate': DocumentDatabase.currentRate,
          'area': DocumentDatabase.currentArea,
          'city': DocumentDatabase.currentCity,
